...
const BusinessLogin = () => {
  ...
  const { businessUser, loginBusiness } = useContext(AuthContext);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const loggedInAccount = await loginBusiness(email, password);
      console.log('Logged in business account:', loggedInAccount.name);
      navigate('/business/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen bg-white font-sans text-gray-900 relative selection:bg-[#00b67a] selection:text-white overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1300px] mx-auto min-h-screen flex flex-col lg:flex-row items-center py-12 px-6 sm:px-12 lg:px-24">
        ...
        <div className="w-full lg:w-[45%] lg:pr-16 mb-12 lg:mb-0">
          <button onClick={() => navigate('/business')} className="inline-flex items-center text-[13px] font-semibold text-gray-600 hover:text-black transition-colors mb-12">
          ...
          <h2 className="text-[40px] font-bold text-gray-900 leading-[1.1] mb-6">
        ...
        <div className="w-full lg:w-[55%] flex justify-end">
          <div className="bg-white rounded-[24px] shadow-[0_20px_60px_rgb(0,0,0,0.12)] p-8 sm:p-12 w-full max-w-[540px] border border-gray-100/50">
            <h1 className="text-[28px] font-semibold text-gray-900 mb-8 mt-2">
              Business Log in
            </h1>
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <a href="#" className="text-sm font-semibold text-[#00b67a] hover:underline">Forgot password?</a>
                </div>
                <input 
                  type="password" 
                  name="password"
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-[#1c55fd] text-white font-semibold py-4 rounded-[8px] hover:bg-[#1541c4] transition-all mt-4 text-base shadow-lg shadow-blue-200 disabled:opacity-50 flex items-center justify-center gap-2"
              >
            ...
            <div className="mt-8 pt-8 border-t border-gray-100 space-y-4">
              <p className="text-center text-gray-600 font-medium">
                ...
                <Link to="/business/signup" className="text-[#00b67a] font-semibold hover:underline">
                  Create a free account
                </Link>
              </p>
              <p className="text-center text-[13px] text-gray-400 font-medium">
                Looking for the consumer login?{' '}
                <Link to="/login" className="text-[#00b67a] font-semibold hover:underline">
                  Go there instead
                </Link>
              </p>
            </div>
